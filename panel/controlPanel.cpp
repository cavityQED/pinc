...
void controlPanel::setupModeGroup()
{
	...
	connect(m_jog_button,	&QPushButton::clicked, [this] {modeChange(panelMode_t::JOG);	});
	connect(m_home_button,	&QPushButton::clicked, [this] {modeChange(panelMode_t::HOME);	});
	connect(m_auto_button,	&QPushButton::clicked, [this] {modeChange(panelMode_t::AUTO);	});
	connect(m_MDI_button,	&QPushButton::clicked, [this] {modeChange(panelMode_t::MDI);	});
	connect(m_edit_button,	&QPushButton::clicked, [this] {modeChange(panelMode_t::EDIT);	});
	...
	//Add the buttons to a 3x2 grid layout
...
void controlPanel::setupJogGroup()
{
	...
	connect(m_jog100_button,	&QPushButton::clicked, [this] {jogSpeed(100);	jogDistance(1.0);});
	connect(m_jog10_button,		&QPushButton::clicked, [this] {jogSpeed(10);	jogDistance(0.5);});
	connect(m_jog1_button,		&QPushButton::clicked, [this] {jogSpeed(1);		jogDistance(0.1);});
	...
	m_jogYpos_button->setShortcut(Qt::Key_W);
	m_jogXneg_button->setShortcut(Qt::Key_A);
	m_jogYneg_button->setShortcut(Qt::Key_S);
	m_jogXpos_button->setShortcut(Qt::Key_D);

	connect(m_jogXpos_button,	&QPushButton::clicked, [this] {axisButton(X_AXIS, true);	});
	connect(m_jogXneg_button,	&QPushButton::clicked, [this] {axisButton(X_AXIS, false);	});
	connect(m_jogYpos_button,	&QPushButton::clicked, [this] {axisButton(Y_AXIS, true);	});
	connect(m_jogYneg_button,	&QPushButton::clicked, [this] {axisButton(Y_AXIS, false);	});
...
void controlPanel::setupControlGroup()
{
	...
	connect(m_hold_button,	&QPushButton::clicked, this, &controlPanel::hold);
